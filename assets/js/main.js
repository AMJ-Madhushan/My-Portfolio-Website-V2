(function ($) {
  ...
  $documentOn.ready( function() {
    ...
    //>> Project Filter Chips Start <<//
    const $projectItems = $(".project-item-col");
    $projectItems.addClass("active-card");

    $(document).on("click", ".project-filter .filter-btn", function (e) {
      e.preventDefault();
      ...
      $projectItems.each(function () {
        const $col = $(this);
        const category = $col.find(".project-items-2").data("category");

        if (filter === "all" || category === filter) {
          $col.removeClass("project-item-hidden").stop(true, true).css("display", "flex").fadeIn(220);
          $col.find(".project-items-2").removeClass("project-item-hidden");
        } else {
          $col.addClass("project-item-hidden").stop(true, true).fadeOut(220);
        }
      });
    });
